...
from src.podcast.script_generator_oss import PodcastScriptGeneratorOSS
...
st.set_page_config(
    page_title="Podsite OSS - AI Podcast Generator",
    page_icon="🎙️",
    layout="wide",
...
def initialize_generators():
    ...
    try:
        # Initialize OSS script generator
        st.session_state.script_generator = PodcastScriptGeneratorOSS(
            ollama_url=st.session_state.ollama_url,
            model_name=st.session_state.ollama_model
        )

        # Assume Ollama is running - skip connection check
...
def generate_podcast(selected_source_name: str, podcast_style: str, podcast_length: str):
    ...
    try:
        with st.spinner("✍️ Generating podcast script with Ollama..."):
...
def render_studio_tab():
    ...
    if not st.session_state.sources:
        ...
        st.markdown("Create an AI-generated podcast discussion from your content using Ollama")
...
def main():
    ...
    <div style="text-align: center;">
        <h1 style="color: #ffffff; margin: 0;">🎙️ Podsite<span class="oss-badge">OPEN SOURCE</span></h1>
        <p style="color: #a0aec0; font-size: 18px;">Transform Web Content into Engaging Podcasts with Local AI</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="text-align: center; color: #a0aec0; font-size: 12px;">
        Podsite - AI-Powered Podcast Generation | Built with Streamlit & Ollama
    </div>
    """, unsafe_allow_html=True)
